...
class SVProfileModal extends Component {
  ...
  async fetchCTDData() {
    if (
      ...
      const ctd_data = await axios
        .get(
          `${API_ROOT_URL}/api/v1/event_aux_data?datasource=vehicleRealtimeCTDData&startTS=${this.props.lowering.start_ts}&stopTS=${this.props.lowering.lowering_additional_meta.milestones.lowering_on_bottom}`,
          {
            headers: {
      ...
      this.setState({ status_msg: '', ctd_data })
      return
    }
  ...
  renderSVProfile(ctd_data) {
    const baseOptions = {
      title: { text: null },
      chart: { inverted: true, height: 600 },
      legend: { enabled: false },
      xAxis: { title: { text: 'Depth [m]' } },
      yAxis: { lineWidth: 1, title: { text: 'Sound Velocity [m/s]' } }
    }

    if (!ctd_data) {
      return <HighchartsReact highcharts={Highcharts} options={{ ...baseOptions, series: [{ data: [] }] }} oneToOne={true} />
    }

    const profile_data = ctd_data
      .map((data_point) => {
        let depth = data_point.data_array.find((value) => value.data_name == 'depth')
        depth = depth ? parseFloat(depth.data_value) : null
    ...
    const svProfileOptions = {
      ...baseOptions,
      series: [{ data: profile_data, states: { hover: { enabled: false } } }],
      tooltip: { enabled: false },
      xAxis: { ...baseOptions.xAxis, gridLineWidth: 1 }
    }

    return <HighchartsReact highcharts={Highcharts} options={svProfileOptions} oneToOne={true} />
  }
  ...
  render() {
    ...
    if (lowering) {
      const svProfile = this.renderSVProfile(this.state.ctd_data)
          ...
          <Modal.Body>
            ...
            <div className='mt-2'>
              ...
              <span className='float-end'>
                <Button variant='outline-primary' size='sm' onClick={this.exportDataToFile} disabled={!this.state.ctd_data}>
                  Export
                </Button>
              </span>
            </div>
