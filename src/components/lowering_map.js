    ...
    if(this.props.event.events && this.props.event.events.length > 0){
      ...
      let eventList = this.props.event.events.map((event, index) => {
        if(index >= (this.state.activePage-1) * maxEventsPerPage && index < (this.state.activePage * maxEventsPerPage)) {
          ...
          return (<ListGroup.Item className="py-1 event-list-item" key={event.id} active={active} ><span className="flex-grow-1 pseudo-link" onClick={() => this.handleEventClick(index)} >{`${event.ts} <${event.event_author}>: ${event.event_value} ${eventOptions}`}</span><span className="float-right flex-none">{eventDetails} {eventComment}</span></ListGroup.Item>);
