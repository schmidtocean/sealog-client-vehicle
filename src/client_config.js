...
//   `http://${window.location.hostname}:8100/sealog-server`;
// const WS_ROOT_URL = (typeof window !== 'undefined') &&
//   `ws://${window.location.hostname}:8100/ws`;

const API_ROOT_URL = typeof window !== 'undefined' && `http://10.128.0.26:8100/sealog-server`
const WS_ROOT_URL = typeof window !== 'undefined' && `ws://10.128.0.26:8100/ws`

// Change this if not hosting the client at the webserver's document root i.e. '/sealog/'
const ROOT_PATH = '/sealog-Sub/'
...
const CRUISE_ID_PLACEHOLDER = 'i.e. FKt210101'
...
const LOWERING_ID_PLACEHOLDER = 'i.e. S0319'

// RegExp object to use when verifying LoweringID i.e. RegExp(/^CHR[0-9]{4}$/)
const LOWERING_ID_REGEX = RegExp(/^S[0-9]{4}$/)
...
const CUSTOM_LOWERING_NAME = ['dive', 'dives']
